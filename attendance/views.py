...
import csv
import json
from .models import Devotee, Sabha, Attendance, UserProfile
...
def get_user_sabha_types(user):
    """Get allowed sabha types for user"""
    if user.is_superuser:
        return [choice[0] for choice in Devotee.SABHA_CHOICES]
    
    try:
        profile, created = UserProfile.objects.get_or_create(user=user)
        if not profile.allowed_sabha_types:
            # If no sabha types assigned, give access to all by default
            profile.allowed_sabha_types = [choice[0] for choice in Devotee.SABHA_CHOICES]
            profile.save()
        return profile.allowed_sabha_types
    except Exception:
        return [choice[0] for choice in Devotee.SABHA_CHOICES]
...
def dashboard(request):
    ...
    if allowed_sabha_types:
        total_devotees = Devotee.objects.filter(sabha_type__in=allowed_sabha_types).count()
        recent_sabhas = Sabha.objects.filter(sabha_type__in=allowed_sabha_types)[:5]
    else:
        total_devotees = 0
        recent_sabhas = []
    ...
    if allowed_sabha_types:
        total_attendance_records = Attendance.objects.filter(sabha__sabha_type__in=allowed_sabha_types).count()
        present_records = Attendance.objects.filter(sabha__sabha_type__in=allowed_sabha_types, status='present').count()
    else:
    ...
    if allowed_sabha_types:
        this_week_sabhas = Sabha.objects.filter(sabha_type__in=allowed_sabha_types, date__range=[week_start, week_end]).count()
    else:
        this_week_sabhas = 0
    ...
    for i in range(4):
        ...
        if allowed_sabha_types:
            sabhas_count = Sabha.objects.filter(sabha_type__in=allowed_sabha_types, date__range=[week_start, week_end]).count()
...
def devotee_list(request):
    ...
    if search_query:
        if search_type == 'id':
            devotees = devotees.filter(devotee_id__icontains=search_query)
        elif search_type == 'name':
            devotees = devotees.filter(name__icontains=search_query)
        elif search_type == 'phone':
            devotees = devotees.filter(contact_number__icontains=search_query)
        elif search_type == 'type':
            devotees = devotees.filter(devotee_type__icontains=search_query)
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        devotees_data = [{
            'id': d.id,
            'devotee_id': getattr(d, 'devotee_id', 'N/A'),
            'name': d.name,
            'contact_number': d.contact_number,
            'sabha_type_display': d.get_sabha_type_display(),
            'devotee_type': getattr(d, 'devotee_type', 'N/A'),
            'join_date': d.join_date.strftime('%Y-%m-%d') if d.join_date else 'N/A',
...
def devotee_add(request):
    ...
    if request.method == 'POST':
        form = DevoteeForm(request.POST, request.FILES)
        if form.is_valid():
            ...
            if cropped_photo and cropped_photo.startswith('data:image'):
                ...
                photo_file = ContentFile(photo_data, name=f'temp.{ext}')
                
                devotee_data = {
                    'name': devotee.name,
                    'contact_number': devotee.contact_number
                }
                photo_url = upload_devotee_photo(photo_file, devotee_data)
                if photo_url:
                    devotee.photo_url = photo_url
        ...
        form = DevoteeForm()
        # Limit sabha type choices
        form.fields['sabha_type'].choices = [(choice[0], choice[1]) for choice in Devotee.SABHA_CHOICES if choice[0] in allowed_sabha_types]
...
def devotee_edit(request, pk):
    ...
    devotee = get_object_or_404(Devotee, pk=pk)
    if request.method == 'POST':
        form = DevoteeForm(request.POST, request.FILES, instance=devotee)
        if form.is_valid():
            devotee = form.save(commit=False)
...
def devotee_delete(request, pk):
    ...
    if request.method == 'POST':
        devotee.delete()
        messages.success(request, 'Devotee deleted successfully!')
        return redirect('devotee_list')
    return render(request, 'attendance/devotee_confirm_delete.html', {'devotee': devotee})
...
def sabha_add(request):
    ...
    if request.method == 'POST':
        ...
        form = SabhaForm()
        # Limit sabha type choices
        form.fields['sabha_type'].choices = [(choice[0], choice[1]) for choice in Sabha.SABHA_CHOICES if choice[0] in allowed_sabha_types]
